...
## The anaJ gene has both 720 and 210 bp annotations between the samples
## The reference species 2 genome, has the shorter 210bp anaJ gene
## Due to the uncertainty in the true anaJ sequence, it was excluded from the atx gene cluster analysis
...
## Matching the species present in each sample
...
table(atx_gi_df$name, atx_gi_df$SNV_mbp == 0)
...
ggplot(data= snv_postions_counts, aes(x= n)) +
  ...
  facet_grid(name ~.) +
  theme_strains
ggsave(last_plot(), filename= "atx_SNV_sharing_hist.jpg", path= "Output_figures", width= 8, height= 6, units= "in", dpi= 320)
