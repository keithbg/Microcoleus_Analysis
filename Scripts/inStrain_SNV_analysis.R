...
snv_list_morph34 <- map(snv_files, function(x) suppressMessages(read_tsv(file.path(in_dir, x))) %>% 
                           mutate(pos_id= str_c(scaffold, position, sep= "-")) %>% 
                           filter(morphia == 3 | morphia == 4) %>% 
                           select(pos_id, conBase)) %>% 
  setNames(str_replace(snv_files, ".tsv", ""))

snv_list_morph12 <- map(snv_files, function(x) suppressMessages(read_tsv(file.path(in_dir, x))) %>% 
                          mutate(pos_id= str_c(scaffold, position, sep= "-")) %>% 
                          filter(morphia == 1 | morphia == 2) %>% 
                          select(pos_id, conBase)) %>% 
  setNames(str_replace(snv_files, ".tsv", ""))

snv_list_morph1 <- map(snv_files, function(x) suppressMessages(read_tsv(file.path(in_dir, x))) %>% 
                           mutate(pos_id= str_c(scaffold, position, sep= "-")) %>% 
                           filter(morphia == 1) %>% 
                           select(pos_id, conBase)) %>% 
  setNames(str_replace(snv_files, ".tsv", ""))

snv_list_morph2 <- map(snv_files, function(x) suppressMessages(read_tsv(file.path(in_dir, x))) %>% 
                         mutate(pos_id= str_c(scaffold, position, sep= "-")) %>% 
                         filter(morphia == 2) %>% 
                         select(pos_id, conBase)) %>% 
  setNames(str_replace(snv_files, ".tsv", ""))
...
snv_df_conBase_morph12 <- bind_rows(snv_list_morph12, .id= "sample") %>% 
  mutate(site= do.call(rbind, str_split(.$sample, "\\."))[, 1],
         species= do.call(rbind, str_split(.$sample, "\\."))[, 2]) %>% 
  inner_join(species_lookup, ., by= c("site", "species")) %>% # Filter to only include species recovered from each site
  mutate(conBase= ifelse(conBase == "A", 1,
                         ifelse(conBase == "T", 2,
                                ifelse(conBase == "C", 3, 4))))
snv_df_conBase_morph34 <- bind_rows(snv_list_morph34, .id= "sample") %>% 
  mutate(site= do.call(rbind, str_split(.$sample, "\\."))[, 1],
         species= do.call(rbind, str_split(.$sample, "\\."))[, 2]) %>% 
  inner_join(species_lookup, ., by= c("site", "species")) %>% # Filter to only include species recovered from each site
  mutate(conBase= ifelse(conBase == "A", 1,
...
snv_df_conBase_morph1 <- bind_rows(snv_list_morph1, .id= "sample") %>% 
  mutate(site= do.call(rbind, str_split(.$sample, "\\."))[, 1],
         species= do.call(rbind, str_split(.$sample, "\\."))[, 2]) %>% 
  inner_join(species_lookup, ., by= c("site", "species")) %>% # Filter to only include species recovered from each site
  mutate(conBase= ifelse(conBase == "A", 1,
                         ifelse(conBase == "T", 2,
                                ifelse(conBase == "C", 3, 4))))
table(snv_df_conBase_morph1$site == "PH2015_12U")

snv_df_conBase_morph2 <- bind_rows(snv_list_morph2, .id= "sample") %>% 
  mutate(site= do.call(rbind, str_split(.$sample, "\\."))[, 1],
         species= do.call(rbind, str_split(.$sample, "\\."))[, 2]) %>% 
  inner_join(species_lookup, ., by= c("site", "species")) %>% # Filter to only include species recovered from each site
  mutate(conBase= ifelse(conBase == "A", 1,
...
snv_matrix_conBase_morph1 <- snv_df_conBase_morph1 %>% 
  select(site, pos_id, conBase) %>% 
  spread(key= pos_id, value= conBase, fill= 0)

snv_matrix_conBase_morph2 <- snv_df_conBase_morph2 %>% 
  select(site, pos_id, conBase) %>% 
  spread(key= pos_id, value= conBase, fill= 0)
...
snv_matrix_binary_morph34 <- snv_df_conBase_morph34 %>% 
  select(site, pos_id, conBase) %>% 
  mutate(conBase= ifelse(conBase > 0, 1, 0)) %>% 
  spread(key= pos_id, value= conBase, fill= 0)

snv_matrix_binary_morph12 <- snv_df_conBase_morph12 %>% 
  select(site, pos_id, conBase) %>% 
  mutate(conBase= ifelse(conBase > 0, 1, 0)) %>% 
  spread(key= pos_id, value= conBase, fill= 0)


snv_matrix_binary_morph1 <- snv_df_conBase_morph1 %>% 
  select(site, pos_id, conBase) %>% 
  mutate(conBase= ifelse(conBase > 0, 1, 0)) %>% 
  spread(key= pos_id, value= conBase, fill= 0)

snv_matrix_binary_morph2 <- snv_df_conBase_morph2 %>% 
  select(site, pos_id, conBase) %>% 
  mutate(conBase= ifelse(conBase > 0, 1, 0)) %>% 
  spread(key= pos_id, value= conBase, fill= 0)

table(unique(snv_df_conBase_morph1$pos_id) %in% unique(snv_df_conBase_morph234$pos_id))
...
write_tsv(snv_matrix_binary_morph1, path= "inStrain/output_tables/snv_pos_binaryM1_sp1.tsv")
...
snv_matrix_binary <- read_tsv("inStrain/output_tables/snv_pos_binary_sp1.tsv")
## SNV sharing
calc_jaccard_dist <- function(){
  snv_sharing <- colSums(snv_matrix_binary_morph1[, -1])
  snv_sharing <- colSums(snv_matrix_binary_morph2[, -1])
  
  table(snv_sharing)
  
  ## Calculate distances between sites
  dist.rows <- vegdist(as.matrix(snv_matrix_binary_morph1[, -1]), method= "jaccard")
  dist.rows <- vegdist(as.matrix(snv_matrix_binary_morph2[, -1]), method= "jaccard")
  dist.rows <- vegdist(as.matrix(snv_matrix_binary_morph34[, -1]), method= "jaccard")
  dist.rows <- vegdist(as.matrix(snv_matrix_binary_morph12[, -1]), method= "jaccard")
  ...
  colnames(snv_dist_mat) <- snv_matrix_binary_morph12$site
  rownames(snv_dist_mat) <- snv_matrix_binary_morph12$site
  #write_tsv(as.data.frame(snv_dist_mat), path= "inStrain/output_tables/snv_pos_M1_jaccard.tsv")
  #write_tsv(as.data.frame(snv_dist_mat), path= "inStrain/output_tables/snv_pos_M2_jaccard.tsv")
  #write_tsv(as.data.frame(snv_dist_mat), path= "inStrain/output_tables/snv_pos_M34_jaccard.tsv")
  #write_tsv(as.data.frame(snv_dist_mat), path= "inStrain/output_tables/snv_pos_M12_jaccard.tsv")
}

calc_jaccard_dist_morph234 <- function(){
...
snv_nmds <- metaMDS(as.matrix(snv_dist_mat[, -1]), maxit= 999, trymax= 250, k= 3, autotransform = FALSE)
...
m1.tib <- read_tsv("inStrain/output_tables/snv_pos_M1_jaccard.tsv")
m1.mat <- as.matrix(1-m1.tib)
rownames(m1.mat) <- names(m1.tib)
colnames(m1.mat) <- names(m1.tib)

## Cluster sites
row_hclust <- hclust(dist.rows, method= "ward.D2")
row_hclust <- hclust(as.matrix(m1.mat), method= "ward.D2")
...
SNV_diss_watershed <- snv_dist_df %>% 
  ...
  labs(x= expression('Watershed area (km'^"2"*")"), y= "SNV similarity (%)") +
